...
- 추천 테크닉: ${results.recommendedTechnique}`;
  ...
  return (
    <section id="basket-calculator-section" className="py-12 sm:py-16 bg-[#FDFBF7] border-y border-[#EDE7DF] text-left">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#FFF4E6] border border-[#FED7AA] text-xs font-bold text-[#C05621] mb-3 shadow-2xs whitespace-nowrap">
            <Calculator className="w-3.5 h-3.5 text-[#F59E0B] shrink-0" />
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-3xl border-2 text-left transition cursor-pointer active:scale-98 ${
                  isSelected
                    ? 'bg-[#FFF8EE] text-[#3E291C] border-[#FF8E72] shadow-sm'
                    : 'bg-white text-[#78614E] border-[#F2E4D0] hover:bg-[#FFF4E6]'
                }`}
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-cute text-sm sm:text-base font-bold text-[#3E291C]">{preset.name}</span>
                  {isSelected && <Sparkles className="w-3.5 h-3.5 text-[#FF8E72]" />}
                </div>
                <div className="text-[11px] text-[#9C7A5E] font-medium">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-6 bg-white p-6 sm:p-7 rounded-3xl border-2 border-[#F5EAD9] shadow-xs space-y-6">
            <div className="flex items-center justify-between pb-3 border-b border-[#F5EAD9]">
              ...
              <span className="text-xs text-[#9C7A5E] font-medium">실시간 자동 연산 중 ✨</span>
            </div>

            {/* Slider 1: Top Rim Diameter */}
            <div className="space-y-2">
              <div className="flex justify-between items-center text-xs">
                <span className="font-bold text-[#1F1A17]">상단 입구 지름 (Rim Diameter)</span>
                <span className="font-mono font-bold text-[#8C5A32] bg-[#FAF7F2] px-2.5 py-0.5 rounded-lg border border-[#EDE7DF]">
                  {diameter} cm
                </span>
              </div>
              <input
                ...
                className="w-full h-2 bg-[#F0EAE1] rounded-lg appearance-none cursor-pointer accent-[#8C5A32]"
              />
              <div className="flex justify-between text-[10px] text-[#A6988D]">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center text-xs">
                <span className="font-bold text-[#1F1A17]">보울 깊이 / 높이 (Depth)</span>
                <span className="font-mono font-bold text-[#8C5A32] bg-[#FAF7F2] px-2.5 py-0.5 rounded-lg border border-[#EDE7DF]">
                  {height} cm
                </span>
              </div>
            ...
            {/* Thickness Select */}
            <div className="space-y-2">
              <label className="text-xs font-bold text-[#1F1A17] block">
                사용할 환심 두께 규격
              </label>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`py-2 px-2.5 rounded-xl border text-xs font-medium transition cursor-pointer ${
                      thickness === item.mm
                        ? 'bg-[#26201B] text-white border-[#26201B] font-bold shadow-2xs'
                        : 'bg-[#FAF7F2] text-[#665B52] border-[#EDE7DF] hover:bg-white'
                    }`}
                  >
                    {item.label}
                  </button>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-xl border text-left transition cursor-pointer ${
                      finishStyle === style.id
                        ? 'bg-[#FAF2E8] border-[#8C5A32] text-[#8C5A32]'
                        : 'bg-[#FAF7F2] border-[#EDE7DF] text-[#665B52] hover:bg-white'
                    }`}
                  >
            ...
            {/* Geometric summary */}
            <div className="p-3.5 rounded-2xl bg-[#FAF7F2] border border-[#EDE7DF] text-xs text-[#665B52] space-y-1">
              <div className="flex justify-between">
                <span>예상 바닥 지름 (Base Diameter):</span>
                <span className="font-mono font-bold text-[#1F1A17]">{Math.round(diameter * 0.62)} cm</span>
              </div>
              <div className="flex justify-between">
                <span>보울 사발 곡선 경사각 (Flare Angle):</span>
                <span className="font-mono font-bold text-[#8C5A32]">약 {results.flareAngle}° (포개짐 최적화)</span>
              </div>
            </div>
          ...
          <div className="lg:col-span-6 space-y-5">
            ...
            <div className="bg-white p-6 sm:p-7 rounded-3xl border border-[#EDE7DF] shadow-2xs space-y-5">
              
              {/* Header with copy button */}
              <div className="flex items-center justify-between pb-3 border-b border-[#F4EFE6]">
                <div>
                  <span className="text-[11px] text-[#8C5A32] font-bold tracking-wide uppercase">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#EDE7DF] bg-[#FAF7F2] hover:bg-white text-xs font-semibold text-[#665B52] transition cursor-pointer whitespace-nowrap"
                >
                  {copied ? (
                    <>
                      <Check className="w-3.5 h-3.5 text-[#7A8B63] shrink-0" />
                      <span className="text-[#7A8B63] whitespace-nowrap">복사 완료!</span>
                    </>
                  ) : (
                    <>
              ...
              <div className="p-4 rounded-2xl bg-[#FAF6F0] border border-[#E8DFC8] flex items-center justify-between">
                <div>
                  <span className="text-xs text-[#8C7B6E] block">기둥이 되는 날대 재단 길이 (1줄당)</span>
                  <div className="font-serif-kr text-2xl sm:text-3xl font-bold text-[#8C5A32]">
                    각 <span className="font-mono">{results.stakeCutLengthCm}</span> cm
                  </div>
                  <span className="text-[11px] text-[#665B52] mt-0.5 block">
                ...
                <div className="text-right font-mono text-xs bg-white px-3 py-2 rounded-xl border border-[#EAE2D7]">
                  <span className="text-[#8C7B6E] block text-[10px]">필요 가닥수</span>
                  <strong className="text-sm text-[#1F1A17]">{results.totalStakes} 가닥</strong>
                </div>
              </div>

              {/* Secondary Metrics Grid */}
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3.5 rounded-2xl bg-[#FAF7F2] border border-[#EDE7DF] text-center">
                  <span className="text-[11px] text-[#8C7B6E] block mb-1">엮음 사릿대</span>
                  <span className="font-mono text-base font-bold text-[#1F1A17]">
                    약 {results.weaverLengthM} m
                  </span>
                  <span className="text-[10px] text-[#A6988D] block mt-0.5">텐션 18% 포함</span>
                </div>

                <div className="p-3.5 rounded-2xl bg-[#FAF7F2] border border-[#EDE7DF] text-center">
                  <span className="text-[11px] text-[#8C7B6E] block mb-1">예상 환심 중량</span>
                  <span className="font-mono text-base font-bold text-[#1F1A17]">
                    약 {results.approxWeightGrams} g
                  </span>
                  <span className="text-[10px] text-[#A6988D] block mt-0.5">소모 환심 총량</span>
                </div>

                <div className="p-3.5 rounded-2xl bg-[#FAF7F2] border border-[#EDE7DF] text-center">
                  <span className="text-[11px] text-[#8C7B6E] block mb-1">미온수 불림</span>
                  <span className="font-mono text-base font-bold text-[#8C5A32]">
                    {results.soakTimeMinutes} 분
                  </span>
                  <span className="text-[10px] text-[#A6988D] block mt-0.5">35~40℃ 침수</span>
                </div>
              </div>

              {/* Dynamic SVG Visual Preview of the Bowl */}
              <div className="p-4 rounded-2xl bg-[#FAF7F2] border border-[#EDE7DF] flex flex-col items-center justify-center">
                <span className="text-[11px] text-[#8C7B6E] mb-2 font-medium">
                ...
                <svg viewBox="0 0 240 100" className="w-52 h-24 drop-shadow-2xs">
                  ...
                  <line x1={120 - diameter * 2.6} y1="10" x2={120 + diameter * 2.6} y2="10" stroke="#8C5A32" strokeWidth="1" strokeDasharray="2 2" />
                  <text x="120" y="8" textAnchor="middle" fontSize="8" fill="#8C5A32" fontWeight="bold">
                    지름 {diameter}cm
                  </text>
                </svg>
              </div>
